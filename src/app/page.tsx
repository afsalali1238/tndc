...
export default async function HomePage() {
  try {
    ...
    if (dishError) {
      console.error("Supabase dishes error:", dishError);
    }
    ...
    if (chefError) {
      console.error("Supabase chefs error:", chefError);
    }
    ...
    return (
      <div className="p-20 text-center font-mono text-sm text-red-600">
        <h1 className="font-bold text-xl mb-4">Caught Server Error on Vercel</h1>
        <p>{error.message || String(error)}</p>
        <pre className="mt-4 text-left p-4 bg-red-50 rounded overflow-x-auto">
          {error.stack}
        </pre>
      </div>
    )
